...
class LazyTreapNode(Generic[Value]):
    """LazyImplicitTreapのノード
    ...
    Attributes:
        ...
        left (Optional[LazyTreapNode[Value]]): 二分探索木のノードの左の子
        right (Optional[LazyTreapNode[Value]]): 二分探索木のノードの右の子
        subtree_size (int): このノードを根とする部分木の要素数
        priority (float): このノードの優先度
        aggregation_value (Value): このノードを根とする部分木の集約値
        lazy (Optional[Value]): 遅延評価用の値. 自身へも反映されていない.
        reversed (bool): 反転のフラグ
    """

    def __init__(self, value: Value):
        self.value = value
        self.left: Optional[LazyTreapNode[Value]] = None
        self.right: Optional[LazyTreapNode[Value]] = None
        self.subtree_size = 1
        self.priority = random()
        self.aggregation_value = value
        self.lazy: Optional[Value] = None
...
class LazyImplicitTreap(Generic[Value]):
    """遅延ImplicitTreap. Treapを配列のように扱う. (再帰)

    Args:
        Value: 二分探索木に格納する値の型

    Attributes:
        root (Optional[LazyTreapNode[Value]]): 二分探索木の根
    ...
    def __init__(
        ...
        lazy_propagator: Callable[[Value, Value], Value],
        add_func: Callable[[Value, Value], Value]
    ):
        """初期化
        """
        self.root: Optional[LazyTreapNode[Value]] = None
        self.aggregate_func = aggregate_func
        self.lazy_propagator = lazy_propagator
        self.lazy_updater = lazy_propagator
        self.add_func = add_func

    def _node_update(self, node: Optional[LazyTreapNode[Value]]):
        """nodeの情報を更新する

        Args:
            node (Optional[LazyTreapNode[Value]]): 更新したいノード
        """
        if node is None:
            return
    ...
    def _node_propagate(self, node: Optional[LazyTreapNode[Value]]):
        """nodeの持っている遅延・反転情報を子に伝播させる

        Args:
            node (Optional[LazyTreapNode[Value]]): 対象ノード
        """
        if node is None:
            return
        ...
        if node.lazy is not None:
            if node.left is not None:
                # node.lazy -> node.left.lazy
                node.left.lazy = self.lazy_propagator(node.lazy, node.left.lazy)
            if node.right is not None:
                # node.lazy -> node.right.lazy
                node.right.lazy = self.lazy_propagator(node.lazy, node.right.lazy)

            # 自身に反映
            node.value = self.lazy_propagator(node.value, node.lazy)

        node.lazy = None
        node.reversed = False
        self._node_update(node)

    def _new_node(self, value: Value) -> LazyTreapNode[Value]:
        return LazyTreapNode[Value](value)

    def _merge(
        self,
        left: Optional[LazyTreapNode[Value]],
        right: Optional[LazyTreapNode[Value]]
    ) -> Optional[LazyTreapNode[Value]]:
        """leftを根とする部分木と, rightを根とする部分木をマージする

        Args:
            left (Optional[LazyTreapNode[Value]]): leftの根
            right (Optional[LazyTreapNode[Value]]): rightの根

        Returns:
            Optional[LazyTreapNode[Value]]: mergeした後の根
        """
        self._node_propagate(left)
        self._node_propagate(right)
    ...
    def _split(
        self,
        root: Optional[LazyTreapNode[Value]],
        key: int
    ) -> tuple[Optional[LazyTreapNode[Value]], Optional[LazyTreapNode[Value]]]:
        """key未満のkeyからなるTreapと, key以上のkeyからなるTreapに分割する

        Args:
            root (Optional[LazyTreapNode[Value]]): 分割したい部分木の根
            key (int): 分割するkey

        Returns:
            tuple[Optional[LazyTreapNode[Value]], Optional[LazyTreapNode[Value]]]: 分割した後の根
        """
        if root is None:
            return None, None

        # 遅延情報の処理
        self._node_propagate(root)
    ...
    def search(self, key: int) -> Optional[LazyTreapNode[Value]]:
        ...
        Returns:
            Optional[LazyTreapNode[Value]]: keyを持つ要素が存在すればその要素を返す. 存在しなければNoneを返す
        """
        # [0..key+1), [key+1..n)
    ...
    def range_update(self, i: int, j: int, value: Value):
        """[i..j)の要素をrange_update_func(value)で上書きする
    ...
    def _inorder(self) -> Generator[LazyTreapNode[Value], None, None]:
        """二分探索木の中間順巡回 (二分探索木の要素を昇順に出力する)

        Yields:
            Generator[LazyTreapNode[Value], None, None]: 二分探索木の中間順巡回で得られる要素
        """
